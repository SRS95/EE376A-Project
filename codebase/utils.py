...
def sample_gaussian(m, v):
    ...
    # Get samples from the standard normal of the correct shape
    q_standard_normal = torch.randn_like(m)

    # Now transform to the mean and variance we actually want
    z = q_standard_normal * torch.sqrt(v) + m
...
def log_normal(x, m, v):
    ...
    Return:
        log_prob: tensor: (batch1, batch2, ...): log probability of each sample. Note
    ...
    log_prob = torch.distributions.normal.Normal(m, torch.sqrt(v)).log_prob(x)
...
def log_normal_mixture(z, m, v):
    ...
    log_prob = log_mean_exp(log_normal(z, m, v), 1)

    '''
    numNormals = m.shape[1]

    log_prob = torch.zeros(z.shape[0],)
    for mixIndex in range(numNormals):
        curr_prob = log_normal(z, m[:, mixIndex, :], v[:, mixIndex, :])
        log_prob += curr_prob / numNormals
    '''
